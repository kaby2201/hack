...
static void incTid(type_tid* tid_p) //inc tid tar imot en peker av type_tid og i sin scope refferer til den som navnet: tid_p
{
	...
	if ++ tid_p->sekunder % 60 == 0 //øker sekunder, også sjekker om sekunder modulus 60 blir 0
	{
		tid_p->sekunder = 0;
		tid_p->minutter++;
	}
...
static int main(void)
{
	...
	while(antallTider < 255) //kjør til listen blir full
	{
		if(~PIND & 0x04) //hvis PIND er trykt (0)
		{
			...
			{
				type_tid* nyTid = (type_tid*)malloc(sizeof(type_tid)); //caster type (type_tid*), malloc reserverer nå minne samme størrelse som type_tid
				...
				tider_p[antallTider++]=nyTid; //lagrer nyTid variabelen til antallTider array
				trykk = 1;
			}
			else
				trykk = 0;
