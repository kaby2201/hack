...
//and it is a struck of loper_type
...
*****************************************************************************************
* This function Putt the info from the nyEvent into the loper[] ,
 loper_p[] and the dynamic list.
*
*****************************************************************************************/
...
//this test test for when we reatch point_dyn_list == head_dyn_list !!
	{
...
//putt the node in to the list her. 
			{
				nyPoini_p->next->prior = temp_p;   
// change the next nodes prior pointer to point to temp_p
				nyPoini_p->prior->next = temp_p;   
// change the prior nodes next pointer to point to temp_p
				temp_p->next = nyPoini_p->next;    
// change the temp_p next pointer to point to the next node in the sorted list
				temp_p->prior = nyPoini_p;         
// change the temp_p prior pointer to point to the prior node in the sorted list.
				nyNode=1;                          
// set nyNode=1 to break out of the while loop to fetch a new node 
//from the list that shall be sorted.
			}
			else
			{
				nyPoini_p = nyPoini_p->next;       
//We are still not at the right place to place the temp_p node, 
//step to the next node in the sorted list
			}
